...
def generate_api_docs(input_text: str) -> str:
    ...
    if rendered:
        logger.info("[api-docs] rendered OpenAPI locally chars=%d", len(rendered))
        return rendered
...
def _render_operation(
    ...
    lines = ["", f"## {method} {path}", "", f"**Описание:** {_one_line(summary)}"]
    parameters = [p for p in path_parameters if isinstance(p, dict)]
    parameters.extend(p for p in operation.get("parameters", []) if isinstance(p, dict))
    if parameters:
...
def _render_request_body(request_body: Any, resolver: SchemaResolver) -> list[str]:
    ...
    if not schema:
        return []
    return ["", "### Тело запроса", "", "```json", json.dumps(_example_from_schema(schema, resolver), ensure_ascii=False, indent=2), "```"]
    ...
    for response in responses.values():
        if isinstance(response, dict):
            ...
            if first_schema:
                break
    if first_schema:
        lines.extend(["", "### Пример ответа", "", "```json", json.dumps(_example_from_schema(first_schema, resolver), ensure_ascii=False, indent=2), "```"])
