...
### 4. Настроить пейринг с Telegram

Когда бот ответит сообщением **"Pairing code: ..."**:
...
Чтобы изменить модель, отредактируй `openclaw/openclaw.json`:
...
**Рекомендуемые модели (доступны на OpenRouter):**
- `openai/gpt-oss-120b` — отличный русский, быстрый ✓ (текущая)
- `meta-llama/llama-3.3-70b-instruct` — хороший русский, дешевле
- `mistralai/mistral-7b-instruct` — быстрый, худший русский
- `anthropic/claude-opus-4-1` — лучший, но требует Anthropic API ключ
...
**Бот вернёт:** Структурированную инструкцию с шагами, требованиями и проверкой

### 2. Проверить инструкцию по стайлгайду

**Сначала отправить стайлгайд:**
```
Это стайлгайд:
...
**Затем отправить инструкцию на проверку:**
```
...
**Бот вернёт:** Подробный отчёт с замечаниями по стайлгайду
...
**Бот вернёт:** Структурированную API документацию в Markdown
...
1. Проверь логи OpenClaw:
```bash
docker compose logs openclaw --tail=50
```

2. Убедись что пейринг пройден (`openclaw pairing approve telegram <КОД>`)

3. Проверь что все агенты работают:
```bash
...
Используй модель с лучшей поддержкой русского:
- `openai/gpt-oss-120b` ✓ (текущая)
- `meta-llama/llama-3.3-70b-instruct` (хорошо)

### Ошибка "No endpoints found for <модель>"

Модель не существует на OpenRouter. Проверь список доступных моделей на https://openrouter.ai/
...
Для развёртывания на сервере используй Docker Compose с переменными окружения:
...
Бот будет доступен в Telegram как `@openclaw_techwriting_bot` (или название твоего бота)
