...
## Ревью страницы документации: «На странице не найден текст»

- Причина: документация рендерится JavaScript (SPA), в HTML нет текста.
  Исправление: прислать текст или экспортированный файл; серверный рендеринг не эмулируется.
- Причина: 401/403 — страница за авторизацией; публичного доступа нет.
  Исправление: прислать файл или открытый URL.
- Причина: ссылка ведет на PDF — бот просит прислать файл вложением, так как PDF
  разбирается парсером файлов.
