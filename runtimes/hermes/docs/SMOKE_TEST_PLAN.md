...
- [ ] Файл `.md` (или `.doc` / `.docx` / `.pdf`) без слов ревью → Markdown-черновик инструкции (spec2doc), без ошибки «Текст постановки не может быть пустым».
- [ ] `проверь <текст документа>` → ревью с итоговой строкой «X критичных / Y важных / Z рекомендаций».
- [ ] `проверь` + файл `.md` / `.docx` → ревью текста из файла (review_file).
- [ ] `проверь https://<сайт документации>/<страница>` → ревью текста страницы; в ответе нет пунктов меню и подвала (review_url).
