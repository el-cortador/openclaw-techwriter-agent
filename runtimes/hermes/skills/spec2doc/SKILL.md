...
description: >
  ...
  (route spec_text, spec_file) или из merge request GitLab (route spec_merge_request).
version: 1.1.0
requires:
...
- Вход: текст постановки аналитика, файл `.pdf` / `.docx` размером до 50 МБ либо ссылка на merge request GitLab.
...
- `instructions.md` — живой системный промпт для постановок (текст, PDF, DOCX).
